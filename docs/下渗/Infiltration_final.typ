...
$ Q_s = c_inf thin h ^ (5/3) $

$
  Q_{s, t+1} = c_inf thin h_{t+1}^(5/3), Q_{s, t} = c_inf thin h_{t}^(5/3), \
  Q_s = (Q_{s, t+1} + Q_{s, t}) / 2 = c_inf / 2 (h_{t+1}^(5/3) + h_{t}^(5/3))
$

$ 
  h_{t} - h_{t+1} = Q_s Delta t  \
  h_{t} - h_{t+1} = (c_inf  Delta t) / 2 (h_{t+1}^(5/3) + h_{t}^(5/3)) \
$

求$h_{t+1}$。

$ F(h_{t+1}) = h_{t} - h_{t+1} - (c_inf Delta t) / 2 (h_{t+1}^(5/3) + h_{t}^(5/3)) $

$ x_"new" = x - F(x) / (F'(x)) $

$ h_{t+1}^{k+1} = h_{t+1}^{k} - F(h_{t+1}) / (F'(h_{t+1}) ) $
