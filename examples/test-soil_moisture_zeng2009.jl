...
function plot_θ(soil)
  ...
  plot!(θ, soil.z[1:N], label="θ_init")
  plot!(soil.θ, soil.z[1:N], label="θ_next")
end
...
begin
  ...
  # plot!(θ, soil.z[1:N], label="θ_init")
  # plot!(θ_next3, soil.z[1:N], label="θ_next (zwt = -2.5 m)")
  # plot!(θ_next5, soil.z[1:N], label="θ_next (zwt = -3.0 m)")
end
