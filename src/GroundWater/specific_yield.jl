...
function specific_yield!(soil::Soil{T}, zwt::T;) where {T<:Real}
  (; N, θ, Sy_d, Sy_r, Sy_e) = soil
  (; θ_sat, ψ_sat, θ_fc, param) = soil.param
  ...
  for i = 1:N+1
    ...
    Sy_r[i] = θ_sat[_i] - θ[_i]    # 补给, 33kPa
    Sy_e[i] = θ_sat[_i] - Retention_θ(_ψ_sat + zwt * 100, param[_i]) # 均衡状态, zwt [m] -> [cm]
