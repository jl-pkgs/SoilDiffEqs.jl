...
# z: 向下为负
function _cal_θE(z1::T, z0::T, zwt::T, ψ_sat::T, θ_sat::T, B::T; use_clamp=true) where {T<:Real}
  C = ψ_sat + zwt
  Δz = (z1 - z0)
...
function cal_θE(z1::T, z0::T, zwt::T, ψ_sat::T, θ_sat::T, B::T; use_clamp=true) where {T<:Real}
  if zwt >= z0
    θE = θ_sat
  elseif z0 < zwt < z1
    d1 = zwt - z0 # 未饱和
    d2 = z1 - zwt   # 饱和
    _θE = _cal_θE(zwt, z0, zwt, ψ_sat, θ_sat, B; use_clamp)
    _θE = (_θE * d1 + θ_sat[j] * d2) / (d1 + d2)
    θE = clamp(_θE, 0, θ_sat)
  elseif zwt <= z1
    θE = _cal_θE(z1, z0, zwt, ψ_sat, θ_sat, B; use_clamp)
  end
  return θE
end
