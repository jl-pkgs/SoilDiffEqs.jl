function soil_moisture_Q0!(soil::Soil{FT}, sink::V, Q0::FT;) where {
  ...
  (; N, ψ, θ, 
    ψ_next, ∂θ∂ψ, K, K₊ₕ, θ_prev, ψ_prev,
    a, b, c, d, e, f) = soil
  dt = soil.dt / 3600 # [s] -> [h]
  ...
  @inbounds for i = 1:N
    if i == 1
      a[i] = 0
      c[i] = -K₊ₕ[i] / Δz₊ₕ[i]
      # b[i] = ∂θ∂ψ[i] * Δz[i] / (0.5 * dt) + K0₊ₕ / dz0₊ₕ - c[i]
      # d[i] = ∂θ∂ψ[i] * Δz[i] / (0.5 * dt) * ψ[i] + K0₊ₕ / dz0₊ₕ * ψ0 + K0₊ₕ - K₊ₕ[i]
      b[i] = ∂θ∂ψ[i] * Δz[i] / (0.5 * dt) - c[i]
      d[i] = ∂θ∂ψ[i] * Δz[i] / (0.5 * dt) * ψ[i] - K₊ₕ[i] - Q0
    elseif i < N
      a[i] = -K₊ₕ[i-1] / Δz₊ₕ[i-1]
      c[i] = -K₊ₕ[i] / Δz₊ₕ[i]
      b[i] = ∂θ∂ψ[i] * Δz[i] / (0.5 * dt) - a[i] - c[i]
      d[i] = ∂θ∂ψ[i] * Δz[i] / (0.5 * dt) * ψ[i] + K₊ₕ[i-1] - K₊ₕ[i]
    elseif i == N
      a[i] = -K₊ₕ[N-1] / Δz₊ₕ[N-1]
      c[i] = 0
      b[i] = ∂θ∂ψ[i] * Δz[i] / (0.5 * dt) - a[i] - c[i]
      d[i] = ∂θ∂ψ[i] * Δz[i] / (0.5 * dt) * ψ[i] + K₊ₕ[N-1] - K[i]
    end
    d[i] -= sink[i]
  end
  ...
  @inbounds for i = 1:N
    if i == 1
      a[i] = 0
      c[i] = -K₊ₕ[i] / (2 * Δz₊ₕ[i])
      # b[i] = ∂θ∂ψ[i] * Δz[i] / dt + K0₊ₕ / (2 * dz0₊ₕ) - c[i]
      # d[i] = ∂θ∂ψ[i] * Δz[i] / dt * ψ[i] + K0₊ₕ / (2 * dz0₊ₕ) * ψ0 +
      #        K0₊ₕ / (2 * dz0₊ₕ) * (ψ0 - ψ[i]) +
      #        c[i] * (ψ[i] - ψ[i+1]) + K0₊ₕ - K₊ₕ[i]
      b[i] = ∂θ∂ψ[i] * Δz[i] / dt - c[i]
      d[i] = ∂θ∂ψ[i] * Δz[i] / dt * ψ[i] - Q0 +
             c[i] * (ψ[i] - ψ[i+1]) - K₊ₕ[i]
    elseif i < N
