function soil_moisture_Q0!(θ::V, ψ::V, sink::V, Q0::T, dz::V, dt, param; fun=van_Genuchten) where {
  ...
  @inbounds for i = 1:n
    if i == 1
      ...
      b[i] = Cap[i] * dz[i] / (0.5 * dt) - c[i]
      d[i] = Cap[i] * dz[i] / (0.5 * dt) * ψ[i] - K₊ₕ[i] - Q0
    elseif i < n
  ...
  @inbounds for i = 1:n
    if i == 1
      ...
      b[i] = Cap[i] * dz[i] / dt - c[i]
      d[i] = Cap[i] * dz[i] / dt * ψ[i] - Q0 + 
             c[i] * (ψ[i] - ψ[i+1]) - K₊ₕ[i]
    elseif i < n
